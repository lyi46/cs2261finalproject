...
# 1 "enemy.h" 1
# 21 "enemy.h"
extern const unsigned short enemyTiles[32768];
...
void updatePlayer4() {
    ...
    if ((~(buttons) & ((1<<7))) && player.y + player.height < g4sh) {
        player.y += player.yVel;
        player.isAnimating = 1;
    ...
    if ((~(buttons) & ((1<<4))) && player.x + player.width < g4sw) {
        player.x += player.xVel;
        player.isAnimating = 1;
...
void drawnoface() {
    ...
    shadowOAM[5].attr0 = ((noface.y - vOff) & 0xFF) | (2<<14) | (0<<13) | (0<<8);
    shadowOAM[5].attr1 = ((noface.x - hOff) & 0x1FF) | (3<<14) | (noface.direction == LEFT ? (1<<12) : 0);
    shadowOAM[5].attr2 = ((((8) * (32) + (0))) & 0x3FF);
